/**
 * Google Apps Script - Google Sheets Database Proxy
 * 
 * Instructions:
  ...
  try {
    ...
    if (action === 'deleteTransaction') {
      ...
      for (let i = 1; i < data.length; i++) {
        if (data[i][0] === id) {
          foundIndex = i + 1; // 1-indexed for sheets row number
          break;
        }
      }
...
/**
 * Utility function to convert existing UTC timestamps in the sheet to local timezone-independent format.
 * Run this once inside the Apps Script Editor toolbar by selecting this function and clicking "Run".
 */
